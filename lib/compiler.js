...
var loadTemplate = function (root, tpl) {
    ...
    if (!fs.existsSync(path)) {
        logger.error('无法读取模板文件' + path);
        process.exit(1);
    }

    var tplContent = fs.readFileSync(path, 'utf8');
    logger.info('读取模板文件 ' + path + ' 完成');
